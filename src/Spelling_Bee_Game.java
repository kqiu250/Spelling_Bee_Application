...
public static void SpellingBeeGame() {

    // Calculate the length - level of difficulty based on the current word count
    // if (CurreantWordCount_Storage.length() <= LEVEL_1) length = random.nextInt(2) + 3; // Random length between 3 and 4
    // else if (CurreantWordCount_Storage.length() <= LEVEL_2) length = random.nextInt(3) + 3; // Random length between 3 and 5
    // else if (CurreantWordCount_Storage.length() <= LEVEL_3) length = random.nextInt(4) + 3; // Random length between 3 and 6
    // else if (CurreantWordCount_Storage.length() <= LEVEL_4) length = random.nextInt(5) + 3; // Random length between 3 and 7
    // else if (CurreantWordCount_Storage.length() <= LEVEL_5) length = random.nextInt(6) + 3; // Random length between 3 and 8
    // else if (CurreantWordCount_Storage.length() <= LEVEL_6) length = random.nextInt(7) + 3; // Random length between 3 and 9
    // else if (CurreantWordCount_Storage.length() <= LEVEL_7) length = random.nextInt(8) + 3; // Random length between 3 and 10
    ...
    while (true) {
        ...
        String userGuess = ((String) scanner.next()).toLowerCase(); // Convert user input to String and lowercase for comparison
        Boolean isCorrect = CheckUserInput(userGuess, word.toLowerCase()); // Convert word to lowercase for comparison
...
public static void definition(String word) {
    ...
    String definition = "a flat surface for transactions, a device or person that tallies numbers, or an opposing action."; // test definition
